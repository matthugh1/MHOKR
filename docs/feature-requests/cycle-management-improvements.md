...
2. **Status transition UX issues:**
   ...
   - No dedicated "Activate" button for DRAFT → ACTIVE transition
...
- [ ] Clicking activates the cycle (DRAFT → ACTIVE)
...
- [ ] Cards are clickable filters (click DRAFT card → filter to DRAFT cycles)
...
- [ ] Transition rules clearly explained:
  - DRAFT → ACTIVE: Start the cycle
  - ACTIVE → LOCKED: Freeze changes (only admins can edit)
  - LOCKED → ARCHIVED: Mark cycle as complete (final state)
...
- Consider splitting: if only status changes → use `/status` endpoint, else use general update
