...
/**
 * Validates all required environment variables for Core API
 * @throws EnvValidationError if any required variable is missing
 */
export function validateCoreApiEnv(): void {
  const errors: string[] = [];
  ...
  // Example:
  // try {
  //   validateRequiredEnv('DATABASE_URL', process.env.DATABASE_URL, 'PostgreSQL connection string');
  // } catch (error) {
  //   if (error instanceof EnvValidationError) {
  //     errors.push(error.message);
  //   }
