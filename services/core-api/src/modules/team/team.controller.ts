...
export class TeamController {
  ...
  async create(@Body() data: { name: string; workspaceId: string }, @Req() req: any) {
    return this.teamService.create(data, req.user.organizationId, req.user.id);
  }
  ...
  async update(@Param('id') id: string, @Body() data: { name?: string }, @Req() req: any) {
    return this.teamService.update(id, data, req.user.organizationId, req.user.id);
  }
  ...
  async delete(@Param('id') id: string, @Req() req: any) {
    return this.teamService.delete(id, req.user.organizationId, req.user.id);
  }
  ...
  async addMember(@Param('id') teamId: string, @Body() data: { userId: string; role: string }, @Req() req: any) {
    return this.teamService.addMember(teamId, data, req.user.organizationId, req.user.id);
  }
  ...
  async removeMember(@Param('id') teamId: string, @Param('userId') userId: string, @Req() req: any) {
    return this.teamService.removeMember(teamId, userId, req.user.organizationId, req.user.id);
  }
}
